...
export const uploadData = async (req, res) => {
    try {
        ...
        fs.createReadStream(file.path)
            .pipe(csv())
            .on('data', (row) => {
                // Convert row to data format
                const dataPoint = {
                    date: new Date(row.date),
                    temp_om: row.temp_om ? parseFloat(row.temp_om) : undefined,
                    ...
                    rain_om: row.rain_om ? parseFloat(row.rain_om) : undefined,
                    rain_vc: row.rain_vc ? parseFloat(row.rain_vc) : undefined,
                    pm25: row.pm25 ? parseFloat(row.pm25) : undefined,
